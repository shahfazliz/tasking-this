...
var loader28 = async ({ request, params }) => {
  let { user } = await getUserSession(request), allUsers = (await searchProject2({ userId: user.id })).reduce((accumulator, project) => [...accumulator, ...project.users], []), uniqueUsers = Object.values(allUsers.reduce((accumulator, user2) => ({
    ...accumulator,
    [user2.id]: user2
