...
async function hydrate2(rows) {
  return Promise.all(rows.map(async ({
    ...
    let organizations = await search({ id: organizationId }), createdByUsers = await search3({ id: createdByUserId }), updatedByUsers = await search3({ id: updatedByUserId }), userRoles = await searchUserRole({ roleId: id });
    return new Role({
      id,
      name,
      description,
...
async function addUser4(criteriaObj) {
  try {
    await addUser3(criteriaObj);
  } catch (error) {
    console.error("addUserIntoRole error:", error);
  }
  return this;
}
async function eraseUser4(criteriaObj) {
  try {
    await eraseUser3(criteriaObj);
  } catch (error) {
...
async function addUser5(criteriaObj) {
...
async function eraseUser5(criteriaObj) {
...
// app/model/Topic.tsx
var TABLE_NAME4 = "Topics", TABLE_ATTRIBUTES4 = [
  "name",
  "description",
  "createdByUserId",
  "updatedByUserId"
...
async function search9(criteriaObj) {
  return (await search8(criteriaObj)).map((row) => new Topic(row));
}
...
async function create9(obj) {
  let attributePlaceHolder = Array.from({ length: TABLE_ATTRIBUTES5.length }, (_, index) => "?"), query = `
    ...
    VALUES (${attributePlaceHolder.join(",")})
  `;
  console.log(query), console.log(obj.getAttributeValues()), await db_default.execute(query, obj.getAttributeValues());
}
...
async function search11(criteriaObj) {
  return (await search10(criteriaObj)).map((row) => new TaskStatus(row));
}
...
async function searchResourceTopic(criteriaObj) {
  let { keys, values } = getObjectKeyValues5(criteriaObj), query = `
      ...
      WHERE ${keys.map((column) => `${column}=?`).join(" AND ")}
    `, [rows, response] = await db_default.execute(query, values);
  return rows.length === 0 ? [] : Promise.all(rows.map(async (row) => (await search9({ id: row.topicId }))[0]));
}
async function hydrate6(rows) {
  return Promise.all(rows.map(async ({
    ...
    let createdByUsers = await search3({ id: createdByUserId }), updatedByUsers = await search3({ id: updatedByUserId }), assignedToUsers = await search3({ id: assignedToUserId }), taskStatuses = await search11({ id: taskStatusId }), topics = await searchResourceTopic({ taskId: id });
    return new Task({
      id,
      name,
      description,
...
// app/model/Task.tsx
var TABLE_NAME6 = "Tasks", TABLE_ATTRIBUTES6 = [
  "name",
  "description",
  "createdByUserId",
  "updatedByUserId",
...
async function search13(criteriaObj) {
  return (await search12(criteriaObj)).map((row) => new Task(row));
}
...
async function erase13(criteriaObj) {
  let { keys, values } = getObjectKeyValues6(criteriaObj), attributePlaceholder = keys.map((column) => `${column}=?`).join(" AND "), query = `
...
async function searchTopic(criteriaObj) {
  let query = `
  ...
  `, strUserId = `${criteriaObj.userId}`, [rows, response] = await db_default.execute(query, [strUserId, strUserId]);
  return rows.length === 0 ? [] : Promise.all(rows.map(async (row) => (await search9({ id: row.topicId }))[0]));
}
async function searchTask(criteriaObj) {
  let query = `
    ...
    WHERE createdByUserId=?
  `, strUserId = `${criteriaObj.userId}`, [rows, response] = await db_default.execute(query, [strUserId, strUserId]);
  return rows.length === 0 ? [] : Promise.all(rows.map(async (row) => (await search13({ id: row.taskId }))[0]));
}
...
// app/model/User.tsx
var TABLE_NAME7 = "Users", TABLE_ATTRIBUTES7 = [
  "name",
  "email",
  "hashPassword"
];
...
function getAttributeValues7() {
  return TABLE_ATTRIBUTES7.map((attribute) => this[attribute]);
}
function set7(obj) {
  return Object.keys(obj).forEach((attribute) => this[attribute] = obj[attribute]), this;
}
...
async function search3(criteriaObj) {
  try {
    return await search14(criteriaObj);
  } catch (error) {
...
var searchTopic2 = async (criteriaObj) => {
  try {
    return await searchTopic(criteriaObj);
  } catch (error) {
    return console.error("searchUserTopics error:", error), [];
  }
};
Object.assign(
  User.prototype,
...
var loader2 = async ({ request, params }) => {
  let objs = await search11({ id: params.id });
...
var loader3 = async ({ request, params }) => {
  ...
  return (await search11({ id: params.id }))[0].set({
    name,
    color,
    description,
    updatedBy: user
...
var UserChips = ({ users, organizationId, isManager }) => /* @__PURE__ */ (0, import_jsx_dev_runtime22.jsxDEV)(import_jsx_dev_runtime22.Fragment, { children: users.length === 0 ? "there is no user in this organization" : users.map((user) => /* @__PURE__ */ (0, import_jsx_dev_runtime22.jsxDEV)(
  ...
  let { user } = await getUserSession(request), isManager = (await (user == null ? void 0 : user.roles()) ?? []).some(({ name }) => name === "Manager"), userOrganizations = await searchOrganization2({ userId: user.id }), allUsers = await readAll4();
...
var TABLE_NAME8 = "Permissions", TABLE_ATTRIBUTES8 = [
  "name",
  "description",
  "createdByUserId",
  "updatedByUserId"
...
async function search16(criteriaObj) {
  return (await search15(criteriaObj)).map((row) => new Permission(row));
}
...
}, action9 = async ({ request, params }) => (await erase16({ id: params.id }), (0, import_node12.redirect)("/permissions"));
...
async function addTopic2(criteriaObj) {
  let { keys, values } = getObjectKeyValues7(criteriaObj), attributePlaceHolder = keys.map(() => "?").join(","), query = `
...
async function eraseTopic2(criteriaObj) {
  let { keys, values } = getObjectKeyValues7(criteriaObj), query = `
...
async function hydrate9(rows) {
  return Promise.all(rows.map(async ({
    ...
    let createdByUsers = await search3({ id: createdByUserId }), updatedByUsers = await search3({ id: updatedByUserId }), topics = await searchResourceTopic2({ resourceId: id });
    return new Resource({
      id,
      name,
      description,
...
var TABLE_NAME9 = "Resources", TABLE_ATTRIBUTES9 = [
  "name",
  "description",
  "createdByUserId",
  "updatedByUserId"
...
async function search18(criteriaObj) {
  return (await search17(criteriaObj)).map((row) => new Resource(row));
}
...
}, action12 = async ({ request, params }) => (await erase18({ id: params.id }), (0, import_node16.redirect)("/resources"));
...
var TopicChips = ({ topics, resourceId }) => /* @__PURE__ */ (0, import_jsx_dev_runtime34.jsxDEV)(import_jsx_dev_runtime34.Fragment, { children: topics.length === 0 ? "there is no topic associated to this resource" : topics.map((topic) => /* @__PURE__ */ (0, import_jsx_dev_runtime34.jsxDEV)(
  ...
  let resource = await readAll6(), allTopics = await readAll2();
  ...
  switch (_action) {
    ...
    case ACTION_REMOVE_TOPIC:
      await eraseTopic2({ topicId: values.topicId, resourceId });
      break;
    default:
      break;
  }
...
var UserChips2 = ({ users, projectId, isManager }) => /* @__PURE__ */ (0, import_jsx_dev_runtime40.jsxDEV)(import_jsx_dev_runtime40.Fragment, { children: users.length === 0 ? "there is no user in this project" : users.map((user) => /* @__PURE__ */ (0, import_jsx_dev_runtime40.jsxDEV)(
  ...
  let { user } = await getUserSession(request), isManager = (await (user == null ? void 0 : user.roles()) ?? []).some(({ name }) => name === "Manager"), userProjects = await searchProject2({ userId: user.id }), allUsers = await readAll4();
  ...
  switch (_action) {
    ...
    case ACTION_REMOVE_USER2:
      await eraseUser5({ userId: values.userId, projectId });
      break;
    default:
      break;
  }
...
var loader20 = async ({ request, params }) => {
  let objs = await search9({ id: params.id });
  return (0, import_node28.json)(objs[0]);
}, action22 = async ({ request, params }) => (await erase8({ id: params.id }), (0, import_node28.redirect)("/topics"));
...
var loader21 = async ({ request, params }) => {
  ...
  let { user } = await getUserSession(request), { name, description } = sanitizeData({ formData: await request.formData() });
  return (await search9({ id: params.id }))[0].set({
    name,
    description,
    updatedBy: user
...
var ProjectChips = ({ projects, topicId }) => /* @__PURE__ */ (0, import_jsx_dev_runtime51.jsxDEV)(import_jsx_dev_runtime51.Fragment, { children: projects.length === 0 ? "there is no project associated to this topic" : projects.map((project) => /* @__PURE__ */ (0, import_jsx_dev_runtime51.jsxDEV)(
  ...
  lineNumber: 86,
  columnNumber: 11
}, this), loader22 = async ({ request, params }) => {
  let { user } = await getUserSession(request), userTopics = await searchTopic2({ userId: user.id }), userProjects = await searchProject2({ userId: user.id });
...
function DeleteRole2() {
  let isSubmitting = (0, import_react51.useTransition)().state === "submitting", { role, organizations } = (0, import_react51.useLoaderData)();
...
function CreateRole() {
  let isSubmitting = (0, import_react52.useTransition)().state === "submitting";
...
var import_jsx_dev_runtime58 = require("react/jsx-dev-runtime"), ACTION_ADD_USER3 = "add-user", ACTION_REMOVE_USER3 = "remove-user";
function AllRoles() {
  let { roles: roles2, allUsers, isAdmin } = (0, import_react53.useLoaderData)();
  return /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(import_jsx_dev_runtime58.Fragment, { children: [
    ...
    /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(Rows7, { roles: roles2, allUsers, isAdmin }, void 0, !1, {
      fileName: "app/routes/roles/index.tsx",
      lineNumber: 47,
      columnNumber: 5
    }, this),
...
var UserChips3 = ({ users, roleId, isAdmin }) => /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(import_jsx_dev_runtime58.Fragment, { children: users.length === 0 ? "there is no user in this role" : users.map((user) => /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(
  Chip,
  {
    actionName: ACTION_REMOVE_USER3,
  ...
  lineNumber: 53,
  columnNumber: 11
}, this), UserSelectOptions3 = ({ roleId, allUsers }) => /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(import_react53.Form, { method: "post", children: [
  /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)("input", { type: "hidden", name: "roleId", value: roleId }, void 0, !1, {
    fileName: "app/routes/roles/index.tsx",
    lineNumber: 76,
    columnNumber: 7
  }, this),
  /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(LabelSelect, { name: ACTION_ADD_USER3, options: allUsers }, void 0, !1, {
    fileName: "app/routes/roles/index.tsx",
    lineNumber: 77,
    columnNumber: 7
  }, this),
  /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(
    "button",
    {
      type: "submit",
      name: "_action",
      value: ACTION_ADD_USER3,
      "aria-label": ACTION_ADD_USER3,
      children: "add"
    },
    void 0,
    !1,
  ...
  lineNumber: 75,
  columnNumber: 5
}, this), Rows7 = ({ roles: roles2, allUsers, isAdmin }) => /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(import_jsx_dev_runtime58.Fragment, { children: roles2.map(({
  id,
  name,
  description,
  // organization,
  ...
  updatedAt,
  users
}, index) => /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)("details", { children: [
  /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)("summary", { className: "with-control-button", children: [
    /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)("span", { children: [
      index + 1,
      ". ",
      name
    ] }, void 0, !0, {
    ...
    isAdmin && /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(UpdateNavLink, { to: `./update/${id}` }, void 0, !1, {
      fileName: "app/routes/roles/index.tsx",
      lineNumber: 111,
      columnNumber: 27
    }, this),
    isAdmin && /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(DeleteNavLink, { to: `./delete/${id}` }, void 0, !1, {
      fileName: "app/routes/roles/index.tsx",
      lineNumber: 112,
      columnNumber: 27
    }, this)
  ...
  /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)("ul", { children: [
    /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)("li", { children: [
      "Description: ",
      description
    ] }, void 0, !0, {
      fileName: "app/routes/roles/index.tsx",
    ...
    /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)("li", { children: [
      "Users: ",
      /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(UserChips3, { users, roleId: id, isAdmin }, void 0, !1, {
        fileName: "app/routes/roles/index.tsx",
        lineNumber: 118,
        columnNumber: 26
      }, this)
  ...
  isAdmin && /* @__PURE__ */ (0, import_jsx_dev_runtime58.jsxDEV)(UserSelectOptions3, { allUsers, roleId: id }, void 0, !1, {
    fileName: "app/routes/roles/index.tsx",
    lineNumber: 120,
    columnNumber: 25
  }, this)
  ...
  lineNumber: 91,
  columnNumber: 11
}, this), loader26 = async ({ request }) => {
  let { user } = await getUserSession(request), isAdmin = (await (user == null ? void 0 : user.roles()) ?? []).some(({ name }) => name === "Admin"), allRoles = await readAll(), allUsers = await readAll4();
  return (0, import_node35.json)({
    roles: allRoles,
    allUsers,
    isAdmin
  });
}, action29 = async ({ request }) => {
  let { user } = await getUserSession(request), { _action, roleId, ...values } = sanitizeData({ formData: await request.formData() });
  switch (_action) {
    case ACTION_ADD_USER3:
      await addUser4({
        createdByUserId: user.id,
        roleId,
        userId: values[ACTION_ADD_USER3]
      });
      break;
    case ACTION_REMOVE_USER3:
      await eraseUser4({ userId: values.userId, roleId });
      break;
    default:
      break;
  }
...
var loader27 = async ({ request, params }) => {
  let objs = await search13({ id: params.id });
  return (0, import_node36.json)(objs[0]);
}, action30 = async ({ request, params }) => (await erase12({ id: params.id }), (0, import_node36.redirect)("/tasks"));
...
var loader28 = async ({ request, params }) => {
  let { user } = await getUserSession(request), allUsers = (await searchProject2({ userId: user.id })).reduce((accumulator, project) => [...accumulator, ...project.users], [user]), uniqueUsers = Object.values(allUsers.reduce((accumulator, user2) => ({
    ...accumulator,
    [user2.id]: user2
  }), {})), taskStatuses = await readAll3(), objs = await search13({ id: params.id });
  ...
  let { user } = await getUserSession(request), {
    ...
    urgency,
    ...values
  } = sanitizeData({ formData: await request.formData() }), searchedUsers = await search3({ id: values["assign-to"] }), taskStatuses = await search11({ id: status });
  return (await search13({ id: params.id }))[0].set({
    name,
    description,
...
var loader29 = async ({ request, params }) => {
  let { user } = await getUserSession(request), allUsers = (await searchProject2({ userId: user.id })).reduce((accumulator, project) => [...accumulator, ...project.users], [user]), uniqueUsers = Object.values(allUsers.reduce((accumulator, user2) => ({
    ...accumulator,
    [user2.id]: user2
  }), {})), taskStatuses = await readAll3();
  return (0, import_node38.json)({
    users: uniqueUsers,
    taskStatuses
  });
}, action32 = async ({ request, params }) => {
  let { user } = await getUserSession(request), {
    name,
    description,
    status,
...
var TopicChips2 = ({ topics, taskId }) => /* @__PURE__ */ (0, import_jsx_dev_runtime64.jsxDEV)(import_jsx_dev_runtime64.Fragment, { children: topics.length === 0 ? "there is no topic associated to this task" : topics.map((topic) => /* @__PURE__ */ (0, import_jsx_dev_runtime64.jsxDEV)(
...
}, loader30 = async ({ request }) => {
  let { user } = await getUserSession(request), userTasks = await searchTask({ userId: user.id }), userTopics = await searchTopic({ userId: user.id });
  return (0, import_node39.json)({
    tasks: userTasks,
    allTopics: userTopics
  });
}, action33 = async ({ request }) => {
  ...
  switch (_action) {
    ...
    case ACTION_REMOVE_TOPIC2:
      await eraseTopic({ topicId: values.topicId, taskId });
      break;
    default:
      break;
  }
...
}, action34 = async ({ request, params }) => (await erase14({ id: params.id }), (0, import_node40.redirect)("/users"));
...
var action36 = async ({ request, params }) => {
